...
export interface IChargeResponse {
    ...
    creditCard: {
        ...
        creditCardBrand: 'VISA' | 'MASTERCARD' | 'ELO' | 'DINERS' | 'DISCOVER' | 'AMEX' | 'HIPERCARD' | 'CABAL' | 'BANESCARD' | 'CREDZ' | 'SOROCRED' | 'CREDSYSTEM' | 'JCB' | 'UNKNOWN';
    ...
    split: Array<{
        ...
        totalValue: number;
        cancellationReason: 'PAYMENT_DELETED' | 'PAYMENT_OVERDUE' | 'PAYMENT_RECEIVED_IN_CASH' | 'PAYMENT_REFUNDED' | 'VALUE_DIVERGENCE_BLOCK' | 'WALLET_UNABLE_TO_RECEIVE' | null;
        status: 'PENDING' | 'AWAITING_CREDIT' | 'CANCELLED' | 'DONE' | 'REFUNDED' | 'BLOCKED_BY_VALUE_DIVERGENCE';
        externalReference: string | null;
        description: string | null;
    }>;
    ...
    escrow: {
        ...
        finishReason: 'CHARGEBACK' | 'EXPIRED' | 'INSUFFICIENT_BALANCE' | 'PAYMENT_REFUNDED' | 'REQUESTED_BY_CUSTOMER' | 'CUSTOMER_CONFIG_DISABLED' | null;
    } | null;
    refunds: Array<{
        object: string;
        dateCreated: string;
        status: 'PENDING' | 'AWAITING_CRITICAL_ACTION_AUTHORIZATION' | 'AWAITING_CUSTOMER_EXTERNAL_AUTHORIZATION' | 'CANCELLED' | 'DONE';
        value: number;
